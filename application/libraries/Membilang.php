...
/**
* Library 2 : Library Buatan Sendiri
*/
...
class Membilang
{
	...
	function bilangtigadigit($angka)
	{
		...
		if ($ratusan>1) {
			$retstr .= $this->bilangangka($ratusan) . "ratus ";
		} elseif ($ratusan>0) {
			$retstr .= "seratus ";
		}

		if ($puluhan>1) {
			$retstr .= $this->bilangangka($puluhan) . "puluh ";
		}

		if ($angka>0) {
			if ($puluhan==1) {
				$retstr .= $this->bilangbelasan($angka);
			} else {
				$retstr .= $this->bilangcacah($angka);
			}
		} else {
			if ($puluhan==1) {
				$retstr .= "sepuluh ";
			...
			if (is_numeric($bil)) {
				...
				for ($i=0; $i<sizeof($bilang) ; $i++) {
					...
					if (abs($row[0]<=strlen($bil))) {
						...
						if ($angka>1) {
							$retstr .= $this->bilangtigadigit($angka) . $row[1] . " ";
						} elseif ($angka>0) {
	...
	{
		// Periksa terlebih dahulu apakah angka yang menjadi parameter benar numerik atau bukan
		if (is_numeric($angka)) {
