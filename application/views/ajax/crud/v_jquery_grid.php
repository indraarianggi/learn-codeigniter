...
<script type="text/javascript">
	...
	function editRec(norec) {
		...
		$.ajax({
			url : '<?= site_url('ajax/c_jquery/showrecto/update') ?>',
			type : 'POST',
			data : vardata,
...
<?php
		foreach ($hslquery->result_array() as $row) {
			...
			echo "<td> <a href='#' onClick='editRec({$row["noteman"]});return FALSE;'>Edit</a> </td>";
			echo "<td> <a href='#' onClick='delRec({$row["noteman"]});return FALSE;'>Delete</a> </td>";
			echo "</tr>";
		}
?>
			</table>
...
<script type="text/javascript">
	...
	function delRec(norec) {
		...
		$.ajax({
			url : '<?= site_url('ajax/c_jquery/showrecto/delete'); ?>',
			type : 'POST',
			data : vardata,
			success : function(data) {
				$("#datateman").html(data);
